...
class vision():
    def __init__(self, path, nums, ground_truth, bd_baseline):
        self.width = 0
        self.height = 0
        ...
        # types supported
        self.operations = ['down_sample', 'adjust_frame', 'adjust_quality']

    def apply(self, operation, parameter):
        if operation not in self.operations:
            raise Exception('operation not supported')
        if operation == 'down_sample':
            self.res = float(parameter)
        elif operation == 'adjust_frame':
            self.skip = int(parameter)
        elif operation == 'adjust_quality':
            self.quantizer = int(parameter)
        else:
    ...
    def run(self):
        ...
        self.h264encode()
        self.h264decode()
        if os.path.isfile(self.ground_truth):
            self.application()
        else:
            self.application(self.ground_truth)
        self._f1score = self.get_IoU(self.ground_truth)
        self._bandwidth = self.bandwidth()
        self.clean()
        return self._f1score

    def get_bandwidth(self):
        return self._bandwidth / self.bd_baseline
    ...
    def application(self, baseline=None):
        ...
        self.result = subprocess.check_output(['./extra/video/pd'],
...
TRAIN_BASELINE = '/home/shen/research/RL/WANStream/pd_profile/baseline.txt'
TEST_BASELINE = '/home/shen/research/RL/WANStream/pd_profile/baseline_test.txt'
