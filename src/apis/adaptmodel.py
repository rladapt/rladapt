...
class AdaptModel():
    ...
    def add(self, knob, _range: list, type, discret=None):
    ...
    def train(self, start_from=1, end=None):
        ...
        for epoch in range(start_from + 1, end):
            ...
            bds = list(gauss(i, self.change / 2)
                       for i in np.arange(self.bandvar[0], self.bandvar[1], (self.bandvar[1] - self.bandvar[0]) / 200))
            shuffle(bds)
            ...
            if (epoch and epoch % self.test_interval == 0):
                self.test()
            self.agent.buffer = self.agent.buffer[len(self.agent.buffer):]
